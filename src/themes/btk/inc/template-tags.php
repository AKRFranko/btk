...
function btk_entry_footer() {
	...
	if ( 'post' == get_post_type() ) {
		...
		$categories_list = get_the_category_list( __( ', ', 'btk' ) );
		if ( $categories_list && btk_categorized_blog() ) {
			printf( '<span class="cat-links">' . __( 'Posted in %1$s', 'btk' ) . '</span>', $categories_list );
		}

		/* translators: used between list items, there is a space after the comma */
		$tags_list = get_the_tag_list( '', __( ', ', 'btk' ) );
		if ( $tags_list ) {
			printf( '<span class="tags-links">' . __( 'Tagged %1$s', 'btk' ) . '</span>', $tags_list );
		}
	}
	...
	edit_post_link( __( 'Edit', 'btk' ), '<span class="edit-link">', '</span>' );
}
endif;
